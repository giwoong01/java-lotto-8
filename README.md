...
- Lotto
    - [ ] 1 ~ 45 사이의 중복되지 않는 숫자 6개 로또 정상 생성
    - [ ] 로또 번호의 개수가 6개가 아닐 경우 예외 발생
    - [ ] 로또 번호에 중복된 숫자가 숫자가 있을 경우 예외 발생
    - [ ] 로또 번호가 1 ~ 45 범위를 벗어날 경우 예외 발생
    - [ ] contains() 메소드가 특정 번호의 포함 여부 정상 반환
    - [ ] countMatchingNumbers() 메소드가 당첨 번호와 일치하는 개수 정상 반환
- Lottos
...
- LottoRank
    - [ ] valueOf() 메소드가 일치하는 번호 개수와 보너스 번호 일치 여부에 따라 정상 반환
        ...
        - 2개 이하 일치 -> 낙첨
- LottoResult
    - [ ] calculateProfitRate() 메소드가 총 상금과 구입 금액을 바탕으로 수익률 정상 계산
    - [ ] 당첨금 0일 경우 수익률 0.0 반환
- PurchaseAmount
    - [ ] 정상 구입 금액 객체 생성
    - [ ] 구입 금액에 맞느 로또 개수 정상 반환
    - [ ] 구입 금액이 1000원 단위가 아닌 경우 예외 발생
    - [ ] 구입 금액이 0 또는 음수일 경우 예외 발생
    - [ ] 구입 금액이 숫자가 아닌 문자나 공백일 경우 예외 발생
- BonusNumber
    - [ ] 1 ~ 45 사이의 숫자, 당첨 번호와 중복되지 않는 경우 객체 정상 생성
    - [ ] 보너스 번호가 1 ~ 45 범위를 벗어날 경우 예외 발생
    - [ ] 보너스 번호가 이미 당첨 번호에 포함됭 있는 경우 예외 발생
    - [ ] 보너스 번호가 숫자가 아닌 문자나 공백일 경우 예외 발생
- WinningLotto
    - [ ] determineRank() 메소드가 주어진 로또에 대해 정확한 등수 반환
- WinningNumbers
    - [ ] 1 ~ 45 사이의 중복되지 않은 숫자 6개 당첨 번호 정상 생성
    - [ ] 당첨 번호의 개수가 6개가 아닐 경우 예외 발생
    - [ ] 당첨 번호에 중복된 숫자가 있을 경우 예외 발생
    - [ ] 입력값이 숫자와 쉼표(,) 이외의 문자를 포함할 경우 예외 발생
